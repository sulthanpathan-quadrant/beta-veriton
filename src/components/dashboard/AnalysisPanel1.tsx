...
const createMinimalDataFile = (name: string): DataFile => ({
  ...
  type: name.toLowerCase().endsWith('.json') ? 'json'
       : name.toLowerCase().endsWith('.xlsx') || name.toLowerCase().endsWith('.xls') ? 'excel'
       : 'csv',
  previewData: {
    headers: [],
    rows: [],
 ...
 * pages (Data Quality / NER / Business Logic / ETL Output / AnalysisPanel /
 * PowerBI Dashboard sidebar).
 */
...
// endpoints. Guards against double ".csv" — if `file` already carries the
// extension (e.g. it came from a Databricks /list-datasets response
// elsewhere in the app), it won't be appended again. The default
// (non-Databricks) flow below still blindly appends ".csv", unchanged.
...
export function AnalysisPanel({
    ...
    const fetchKPIs = async () => {
      try {
        ...
        if (useDatabricksRoute) {
          ...
          const dbSource = buildDbSourcePath(userId!, jobId!, file);
  ...
  const handleBuildCustomDashboard = async () => {
    ...
    try {
      ...
      // ── NEW: for Databricks users, datasetName may already carry the
      // ".csv" extension (e.g. if it came from a Databricks
      // /list-datasets response elsewhere), so blindly appending ".csv"
      // like the default flow does below can produce "file.csv.csv".
      // buildDbSourcePath guards against that; for the default
      // (non-Databricks) flow this keeps the original behavior unchanged.
      let blobPath: string;
      if (isDatabricksUser()) {
        blobPath = buildDbSourcePath(userId, jobId, datasetName);
      } else {
        blobPath = `${userId}/${jobId}/${datasetName}`;
      ...
      if (errMsg.includes('404') || errMsg.includes('not found')) {
        ...
        toast.error("Server error while attaching file. Please try again later.", { action: closeToastButton });
      } else {
        toast.error(`Failed to prepare chat session: ${errMsg}`, { action: closeToastButton });
      }
    } finally {
  ...
  // ── Single Select All / Deselect All for both metrics AND measurements of one KPI ──
      ...
      <div className="flex-1 flex items-center justify-center">
        <div className="text-center animate-fade-in">
          ...
          <p className="text-muted-foreground text-sm">
            Detecting KPIs in <span className="text-primary font-medium">{file}</span>
          </p>
        </div>
      </div>
    );
